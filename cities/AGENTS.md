...
- Lire [`docs/architecture-design.md`](./docs/architecture-design.md) avant toute modification structurelle et [`docs/rebuild-playbook.md`](./docs/rebuild-playbook.md) avant une recette ou une reprise.
- Le rapport du rebuild est [`docs/rebuild-report.md`](./docs/rebuild-report.md) ; l'état opérationnel courant reste `REBUILD-STATUS.md`.
...
- JavaScript : responsabilités explicites, modules directs, aucun nouveau patch global ni `MutationObserver` réparateur ; ne pas agrandir le pont de compatibilité sans nécessité démontrée.

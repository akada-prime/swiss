(() => {
  ...
  // The same search input filters the canonical table on Natel and desktop.
  // The keyboard's Search key brings the filtered rows into view.
  document.getElementById('query')?.addEventListener('keydown', event => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    event.currentTarget.blur();
    document.querySelector('.result-line')?.scrollIntoView({ block: 'start' });
  });
